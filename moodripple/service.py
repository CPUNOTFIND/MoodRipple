...
class MoodService:
    ...
    async def create_event(self, atmosphere: str = "") -> dict[str, Any] | None:
        ...
        creative_direction = random.choice([
            "调整自己的头像、签名或主页展示时，做出的两个版本各自暴露了不同的性格倾向",
            "把同一句想说的话录成两种语气，回听后发现它们会传达完全不同的意味",
            "整理自己的收藏、草稿或播放列表时，发现一个关于自身偏好的具体矛盾",
            "裁剪自己准备分享的图片时，发现角落里的细节会让整张图产生另一种解读",
            "设计一个带点顽皮或暧昧留白的表达，但在真正发布前卡在两个不同尺度之间",
            "独自完成一个线上小游戏、测试或创作后，结果迫使自己在两个都舍不得的方案间取舍",
        ])
        result = await self.ai.json(
            ...
            "事件必须以角色自己为唯一行动核心：触发点来自角色自己的操作、创作、整理、选择、观察或自我发现，"
            "具体动作由角色本人完成，意外转折也必须是该动作自然产生的结果。"
            "禁止虚构任何第三方已经给角色发消息、回复、评价、邀请、陪伴、送礼、吃醋或产生暧昧互动；"
            "不得用‘有人’‘朋友’‘某个对话框里的人’规避这条限制，也不得凭空编造群体反馈、投票结果或他人的态度。"
            "平台界面、角色自己的设备、草稿、作品、收藏和公开可观察内容可以作为对象，但不能赋予它们虚构的人际行为。"
            ...
            "也不得假定用户年龄、关系或同意。"
            "recent_events 按从新到旧排列，第一条是最主要的连续性参考，但只可延续角色自身的状态、选择或创作；"
            "旧事件中的第三方互动不得继续作为事实。新事件也不得复刻最近事件的道具、场景、冲突和问题；"
            "若延续旧事件，必须出现由角色自身行动带来的新进展。不得暴露用户信息、原话或身份。"
            "stranger_reference_samples 是陌生人发言经脱敏后的随机样本，高好感用户仅影响抽样概率。"
            "它们与当前用户无关，只能启发问题形式或表达角度，绝不能被改写成‘别人刚刚对角色做了什么’，"
            "不能成为事件事实、主线、关系依据、称呼或记忆，也不能压过最近事件、当前心情和人格设定。"
            "anonymous_atmosphere 也只能轻微影响情绪色彩，不能被写成任何具体人际事件。"
            ...
            "不得只抒情、卖关子或自顾自总结。生成此事件时尚未选定收件人，因此不得写‘想到你’‘你之前说过’"
            "‘我们上次’或任何共同经历、旧约定和关系暗示；只能在当下邀请收件人参与角色自己的问题。"
            "即使脱离所有内部资料也必须完整可懂。self_centered 必须为 true，recipient_assumption 必须为 false；"
            "如果做不到就不要生成。只输出 JSON，不要 markdown："
            '{"description": "第一人称内心事件，80到160字", "delta": number(-15..15), '
            '"topic_intent": "具体可回答的问题，最多60字", '
            '"proactive_seed": "直接对收件人说的完整开场，40到100字", '
            '"self_centered": true, "recipient_assumption": false}。输入：'
            + str({
                "time": now_iso(),
                "current_mood": state["mood"],
                "tags": state["labels"],
        ...
        respects_boundaries = (
            result.get("self_centered") is True
            and result.get("recipient_assumption") is False
        )
        if not description or not topic_intent or not proactive_seed or not respects_boundaries:
            return None
        return {
